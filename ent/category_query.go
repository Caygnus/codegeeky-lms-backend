...
type CategoryQuery struct {
	...
	withInternships *InternshipQuery
	withFKs         bool
...
// QueryInternships chains the current query on the "internships" edge.
func (cq *CategoryQuery) QueryInternships() *InternshipQuery {
	query := (&InternshipClient{config: cq.config}).Query()
...
func (cq *CategoryQuery) Clone() *CategoryQuery {
	...
	return &CategoryQuery{
		...
		withInternships: cq.withInternships.Clone(),
...
// WithInternships tells the query-builder to eager-load the nodes that are connected to
// the "internships" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CategoryQuery) WithInternships(opts ...func(*InternshipQuery)) *CategoryQuery {
	query := (&InternshipClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withInternships = query
	return cq
}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withInternships; query != nil {
		if err := cq.loadInternships(ctx, query, nodes,
			func(n *Category) { n.Edges.Internships = []*Internship{} },
			func(n *Category, e *Internship) { n.Edges.Internships = append(n.Edges.Internships, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CategoryQuery) loadInternships(ctx context.Context, query *InternshipQuery, nodes []*Category, init func(*Category), assign func(*Category, *Internship)) error {
	...
	query.withFKs = true
	query.Where(predicate.Internship(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.InternshipsColumn), fks...))
	}))
